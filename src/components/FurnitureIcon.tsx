...
function DeskSVG({ w, h, color, shape }: { w: number; h: number; color: string; shape: number }) {
  const m = 2;
  // Main horizontal surface: always occupies full width, ~60% height
  const topH = h * 0.6;
  // L extension: right side going downward
  const extX = w * 0.58;
  const extW = w - extX - m;
  const hi = 3;
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`}>
      {/* L extension (drawn first so main surface overlaps at junction) */}
      {shape === 1 && (
...
function PlantSVG({ w, h, color }: { w: number; h: number; color: string; shape: number }) {
...
function PosterSVG({ w, h, color }: { w: number; h: number; color: string; shape: number }) {
  const fc = D(color,50);
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`}>
...
export default function FurnitureIcon({ type, color, shape, shadow = false, preview = false }: Props) {
  ...
  const svgs: Record<FurnitureType, React.ReactNode> = {
    bed: <BedSVG {...p}/>, wardrobe: <WardrobeSVG {...p}/>, desk: <DeskSVG {...p}/>,
    plant: <PlantSVG {...p}/>, poster: <PosterSVG {...p}/>, clock: <ClockSVG {...p}/>,
    door: <DoorSVG {...p}/>, window: <WindowSVG {...p}/>,
  };
